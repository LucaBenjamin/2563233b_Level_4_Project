...
class ImageAutoencoder:
    ...
    def decode(self, latent_dist):
        # Sample from the latent distribution to get the latent vectors
        z = latent_dist.sample()  # This line samples from the Gaussian distribution

        # Decode from latent space to pixel space
        with torch.no_grad():
            decoded = self.model.tiled_decode(z, return_dict=True)
